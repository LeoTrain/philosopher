...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/05 13:53:04 by leberton          #+#    #+#             */
...
static void	setup_philosopher_mutexes(t_program *program, int i)
{
	program->philosophers[i].completion_counter_mutex
		= &program->completion_counter_mutex;
...
int	create_philosophers(t_program *program)
{
	...
	program->philosophers = malloc(sizeof(t_philo)
			* program->args.philosopher_amount);
	...
	{
		setup_philosopher_data(program, i);
		setup_philosopher_mutexes(program, i);
		i++;
	}
	return (SUCCESS);
}
